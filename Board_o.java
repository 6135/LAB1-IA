...
public class Board_o implements Ilayout, Cloneable {
    ...
    public Board_o(String str) throws IllegalStateException{
    ...
    private int[][] movCopy(int[][] old, int newZeroX, int newZeroY){
        int[][] temp = new int[dim][dim];
        for(int c = 0; c < dim; c++)
            for(int r = 0; r < dim; r++){
                if(c == newZeroX && r == newZeroY){
                    temp[r][c] = 0;
                } else if(old[c][r] == 0)
                    temp[r][c] = old[newZeroX][newZeroY];
